...
class DepthwiseViT(nn.Module):
    def __init__(self, in_c: int = 3, num_classes: int = 10, img_size: int = 32, patch: int = 8, dropout: float = 0., num_layers: int = 7, hidden: int = 384, mlp_hidden: int = 384*4, head: int = 8):
        ...
        self.pos_emb = nn.Parameter(
            torch.randn(1, 1, (self.patch**2)+1, hidden))
    ...
    def forward(self, x):
        ...
        out = torch.cat([self.cls_token.repeat(
            out.size(0), 1, 1, 1), self.emb(out)], dim=1)
    ...
    def _to_words(self, x):
        ...
        out = x.unfold(2, self.patch_size, self.patch_size).unfold(
            3, self.patch_size, self.patch_size).reshape(b, c, self.patch**2, -1)
...
if __name__ == "__main__":
    ...
    net = DepthwiseViT(in_c=c, num_classes=10, img_size=h, patch=16,
                       dropout=0.1, num_layers=7, hidden=384//3, head=12, mlp_hidden=384)
    # out = net(x)
    # out.mean().backward()
